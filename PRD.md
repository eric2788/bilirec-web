...
一個專為 Bilibili 直播錄製系統設計的移動端優先 PWA 應用，讓用戶能輕鬆管理錄製任務並瀏覽下載錄製文件。使用標準 Vite + pnpm 構建，支持 HTTP/HTTPS 連接。
...
- **長時間操作** - 顯示加載狀態，防止重複提交
- **HTTP/HTTPS 支持** - 支持 HTTP 和 HTTPS 連接，預設為 `http://localhost:8080`，適合本地開發和內網部署
